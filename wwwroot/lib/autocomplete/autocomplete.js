...
(function ($) {
    var Autocomplete = function () {
        function Autocomplete(element, options) {
            ...
            var containerClasses = 'autocomplete';
            var inputContainerClasses = 'input-group';

            if (this.$input.hasClass('input-sm')) {
                inputContainerClasses += ' input-group-sm';
            }

            this.$input.wrap(`<div class="autocomplete"><div class="${inputContainerClasses}"></div></div>`);
            this.$container = this.$input.parent('.input-group');
            
            console.log(this.$container, this.$container.parent());
            // dropdown button and dropdown menu
            this.$container.append('<button class="btn btn-primary" type="button" tabindex="-1">' + '</button>');
            this.$container.parent(".autocomplete").append('<ul class="items dropdown-menu" style="display:none;"></ul>');

            this.$button = this.$container.find('button');
            this.$items = this.$container.parent(".autocomplete").find('.items');
        ...
        _createClass(Autocomplete, [{
            ...
            value: function setButtonIcon() {
                this.$button.html('<span class="' + (this.open ? 'gg-chevron-up-r' : 'gg-chevron-down-r') + '"></span>');
            }
        }, {
            key: 'search',
